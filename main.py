import base64, zlib, base64, numpy as np, datetime
...
mse_fn = lambda x, y: 1 / (1 + np.mean((d.decode(x) - d.decode(y)) ** 2))
d.conn.execute('''CREATE TABLE IF NOT EXISTS news (news_id INTEGER PRIMARY KEY AUTOINCREMENT, title text, link text, summary text, published text, source text, source_name text, media text, compvec text)''')
d.conn.conn.create_function("mse", 2, mse_fn)
...
async def get_latest(amount: int=10, offset: int=0) -> list[dict]:
    ...
    """
    return [d.parse_row(row) for row in d.conn.fetchall("SELECT * FROM news ORDER BY news_id DESC LIMIT ? OFFSET ?", (amount, offset))]
...
async def get_latest(amount: int=10, offset: int=0) -> list[dict]:
    ...
    """
    return [d.parse_row(row) for row in d.conn.fetchall("SELECT * FROM news ORDER BY published DESC LIMIT ? OFFSET ?", (amount, offset))]
...
async def get_similar(compvec: str, amount: int=10, offset: int=0) -> list[dict]:
    ...
    # With the last 100 entries, find the closest by MSE
    rows = d.conn.fetchall("SELECT * FROM news ORDER BY news_id DESC LIMIT 100")
    
    if len(rows) == 0 or rows is None or type(rows) is not list:
        raise HTTPException(400, detail="No articles in database")
    
    rows = sorted(rows, key=lambda row: mse_fn(row[8], compvec))
    return [d.parse_row(row) for row in rows[offset:offset+amount]]
...
async def get_random(amount: int=10) -> list[dict]:
    ...
    Get random articles
    """
    return [d.parse_row(row) for row in d.conn.fetchall("SELECT * FROM news ORDER BY RANDOM() LIMIT ?", (amount,))]
...
async def search(query: str, source:str=None, amount: int=10, offset: int=0) -> list[dict]:
    ...
    """
    if source is None:
        return [d.parse_row(row) for row in d.conn.fetchall("SELECT * FROM news WHERE title LIKE ? ORDER BY news_id DESC LIMIT ? OFFSET ?", (f"%{query}%", amount, offset))]
    else:
        return [d.parse_row(row) for row in d.conn.fetchall("SELECT * FROM news WHERE title LIKE ? AND source_name = ? ORDER BY news_id DESC LIMIT ? OFFSET ?", (f"%{query}%", source, amount, offset))]
...
async def get_sources() -> list[str]:
    ...
    """
    return [row[0] for row in d.conn.fetchall("SELECT DISTINCT source_name FROM news")]

@app.get("/feed/num_articles")
async def num_articles() -> dict:
    """
    Get number of articles in the database
    """
    result = d.conn.fetchone("SELECT COUNT(*) FROM news")
    return {"num_articles": int(result[0])}
...
if __name__ == "__main__":
    ...
    #uvicorn.run(app, host="0.0.0.0", port=8080 ,ssl_keyfile="privkey.pem", ssl_certfile="fullchain.pem")
